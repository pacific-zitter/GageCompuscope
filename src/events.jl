...
function hybrid_sleep(sleep_time::Float64, threshold::Float64 = .0175)
  ...
  tics_per_sec = 1_000_000_000.  #-- number of tics in one sec
  nano1 = time_ns()                            #-- beginning nano time
  nano2 = nano1 + (sleep_time * tics_per_sec)  #-- final nano time
  min_actual_sleep = .001   #-- do not let the actual_sleep be less than this value
  max_sleep = 86_400_000.   #-- maximum allowed sleep_time parm (100 days in secs)
  min_sleep = .000001000    #-- mininum allowed sleep_time parm (1 micro sec)
